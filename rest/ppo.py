...
from utils import Env_rl_c, BaseController

# Help me to check this PPO Implementation
...
class PPOController(BaseController):
    ...
    def __init__(
        ...
        n_steps: int = 128,      # size of collected data in each rollout
        batch_size: int = 16,     # size of mini-batch
        n_epochs: int = 5,       
    ...
    def _get_action(self, state):
        ...
        # Clamp std
        action_std = action_std.clamp(1e-3, 0.3)
    ...
    def _collect_trajectories(self, n_steps: int):
        ...
        state = np.random.uniform(self.env.state_lbs, self.env.state_ubs)
        ...
        # last state for bootstraping
        if done:
            final_value = np.array([0.0])
        else:
    ...
    def setup(self) -> None:
        ...
        for iteration in range(self.max_iterations):
            ...
            #if self.verbose:
            print(f"Epoch: [{iteration+1}/{self.max_iterations}], total_reward: {total_reward:.2f}")
